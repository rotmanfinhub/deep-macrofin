...
class MultKAN(nn.Module):
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True, random=False, log_history=False):
        '''
    ...
    def unfix_symbolic(self, l, i, j, log_history=False):
        '''
    ...
    def unfix_symbolic_all(self, log_history=False):
        '''
    ...
    def plot(self, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
        ...
        for l in range(neuron_depth - 1):
            ...
            for j in range(n):
                id_ = j
                path = os.path.dirname(os.path.abspath(__file__)) + "/sum_symbol.png"
                im = plt.imread(path)
            ...
            for j in range(n_mult):
                id_ = j + n_sum
                path = os.path.dirname(os.path.abspath(__file__)) + "/mult_symbol.png"
                im = plt.imread(path)
                left = DC_to_NFC([1 / (2 * N) + id_ / N - y2, 0])[0]
                right = DC_to_NFC([1 / (2 * N) + id_ / N + y2, 0])[0]
    ...
    def symbolic_formula(self, floating_digit=2, var=None, normalizer=None, output_normalizer = None):
        '''
        ...
        if simplify:
            return [ex_round(symbolic_acts[-1][i], floating_digit) for i in range(len(symbolic_acts[-1]))], x0
        else:
            return [ex_round(symbolic_acts[-1][i], floating_digit) for i in range(len(symbolic_acts[-1]))], x0
